  ...
  <div>
    <div class="card">
      <div class="card-header">
        <div>
          <h4 class="card-title mb-2 mb-md-0 mb-lg-0">Lista de Suscripciones</h4>
        </div>
      </div>
      ...
      ======================================-->
       ...
       ======================================-->
        <div class="mr-4 mt-1 px-1 m-md-0 m-lg-0">
          ...
          <input v-model="valueSearchSubscriptions" v-on:keyup.enter="search" type="text"
      ...
      <div class="card-body">
        ...
        <div v-if="messageIsSubscriptions && subscriptions.length === 0">
...
import ShowSubscription from "@/components/admin/subscriptions/ShowSubscription.vue";

export default {
  name: "TableSubscriptions",
  data() {
    return {
      plans: [],
      ...
      columnsTable: [
        ...
        {
          field: "subscription_plan_name", key: "subscription_plan_name", title: "Plan", align: "left", width: "10%",
          renderBodyCell: ({row, column, rowIndex}, h) => {
            return <div class="mt-1 d-flex align-items-center">
              <p>{row.subscription_plan_name}</p>
              <p class="text-primary"> {row.subscription_name ? ' - '+ row.subscription_name : ''}</p>
            </div>
          }
        },
        {
          field: "subscription_state", key: "subscription_state", title: "Estado", align: "center", width: "10%",
          renderBodyCell: ({row, column, rowIndex}, h) => {
        ...
        {
          field: "subscription_price", key: "subscription_price", title: "Precio", align: "left", width: "10%",
          renderBodyCell: ({row, column, rowIndex}, h) => {
            return (row.subscription_price).toFixed(2) + '€'
          }
        },
        {
          field: "subscription_period", key: "subscription_period", title: "Período", align: "left", width: "5%",
          renderBodyCell: ({row, column, rowIndex}, h) => {
            switch (row.subscription_period) {
              case 'week':
                return 'Semana'
              case 'month':
                return 'Més'
        ...
        {field: "subscription_patient_name", key: "subscription_patient_name", title: "Paciente", align: "left", width: "10%"},
        {
          ...
          width: "10%",
          renderBodyCell: ({row, column, rowIndex}, h) => {
            return this.$moment(row.subscription_created_at).format('DD MMMM YYYY')
          },
        },
            ...
            return (
              <span>
                <button title="Tooltip directive content"
                        ...
                        disabled={!row.subscription_order}
  ...
  methods: {
    async downloadInvoice(subscription) {
      ...
      window.open(
        this.$config.urlBack + '/api/v1/download-invoice/' + subscription.subscription_order.id + '/' + subscription.subscription_patient_user_id, "_blank"
      )
  ...
  watch: {
    ...
    valueSearchSubscriptions: function (value) {
      if (!value) {
        this.$emit('updateGetSubscriptions')
      }
    },
